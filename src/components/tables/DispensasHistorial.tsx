...
import { Search, Plus, Syringe, Lock } from 'lucide-react'
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
          <div className="hidden lg:grid grid-cols-[auto_2fr_1fr_1fr_1fr_auto] gap-4 px-4 py-2.5 bg-slate-50 border-b border-slate-100 text-xs font-medium text-slate-500 uppercase tracking-wide">
            <span>N°</span>
            <span>Socio</span>
            <span>Genética</span>
            <span>Gramos</span>
          ...
          <div className="divide-y divide-slate-100">
            {filtered.map((d) => (
              <DispensationRow key={d.id} dispensation={d} />
            ))}
          </div>
        </div>
      )}
...
function DispensationRow({ dispensation: d }: { dispensation: DispensationWithMember }) {
  const isAnulacion = d.type === 'anulacion'

  return (
    <div className={cn(
      'grid grid-cols-1 lg:grid-cols-[auto_2fr_1fr_1fr_1fr_auto] gap-2 lg:gap-4 px-4 py-3 items-center',
