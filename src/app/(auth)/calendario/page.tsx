  ...
  return (
    <div className="space-y-5 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Vista mensual de eventos del club</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5">
        ...
        <div className="bg-[#111111] border border-white/[0.06] rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06]">
            <button onClick={prevMonth} className="p-1.5 rounded-lg hover:bg-white/[0.06] transition-colors text-slate-400">
              <ChevronLeft className="w-5 h-5" />
            </button>
            <h3 className="text-base font-semibold text-slate-100">{MONTHS[month]} {year}</h3>
            <button onClick={nextMonth} className="p-1.5 rounded-lg hover:bg-white/[0.06] transition-colors text-slate-400">
              ...
              {cells.map((day, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={cn(
                      'min-h-[72px] p-1.5 border-b border-r border-white/[0.04] last:border-r-0',
                      !day && 'bg-white/[0.02]',
                      day && dayEvents.length > 0 && 'cursor-pointer hover:bg-white/[0.04] transition-colors',
                    )}
                    ...
                    {day && (
                      <>
                        <div className={cn(
                          ...
                          isToday(day) ? 'bg-green-600 text-white' : 'text-slate-400 hover:bg-white/[0.06]',
                        )}>
                          {day}
                        </div>
                        <div className="mt-1 space-y-0.5">
                          ...
                          {dayEvents.length > 2 && (
                            <p className="text-xs text-slate-500 pl-1">+{dayEvents.length - 2} más</p>
                          )}
                        </div>
                      </>
                    )}
        ...
        <div className="space-y-4">
          {/* Leyenda */}
          <div className="bg-[#111111] border border-white/[0.06] rounded-xl p-4 shadow-sm">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Estados</p>
          ...
          {selected ? (
            <div className="bg-[#111111] border border-white/[0.06] rounded-xl p-4 shadow-sm space-y-3">
              <div className="flex items-start justify-between">
                <h4 className="text-sm font-semibold text-slate-100">{selected.name}</h4>
              ...
              <div className="space-y-1.5">
                ...
                {selected.location && (
                  <p className="text-xs text-slate-400 flex items-center gap-1.5">
                    <MapPin className="w-3.5 h-3.5 text-slate-500" />{selected.location}
                  </p>
                )}
              </div>
              <div className="grid grid-cols-2 gap-2 pt-1 border-t border-white/[0.06]">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-[#2DC814]">
                    {new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS', maximumFractionDigits: 0 }).format(selected.total_income ?? 0)}
                  </p>
                </div>
              </div>
              <a href="/eventos" className="block text-center text-xs text-[#2DC814] hover:underline pt-1">
          ...
          <div className="bg-[#111111] border border-white/[0.06] rounded-xl p-4 shadow-sm">
            ...
            {events.length === 0 ? (
              <p className="text-xs text-slate-500 text-center py-2">Sin eventos este mes</p>
            ) : (
              <div className="space-y-2">
                {events.map(ev => (
                  <button
                    ...
                    className={cn(
                      'w-full flex items-center gap-2 text-left px-2 py-1.5 rounded-lg hover:bg-white/[0.04] transition-colors',
